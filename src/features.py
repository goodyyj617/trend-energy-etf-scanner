...
def compute_symbol_features(g: pd.DataFrame) -> pd.DataFrame:
    ...
    g["ma50_slope_10"] = _safe_div(g["ma50"], g["ma50"].shift(10)) - 1
...
def add_signal_surge_v0(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Backtests call this same helper so that signal entry logic stays aligned
